...
class AbstractMuJoCoSimulator:
    ...
    def run(self,
        ...
        for i in range(num_steps + 1):
            ...
            # pos = [
            #     x for p, q in zip(pos_sensor_names, quat_sensor_names)
            #     for x in (
            #             self.mjc_data.sensor(p).data.tolist()
            #             + self.mjc_data.sensor(q).data.tolist()
            #     )
            # ]
            # vel = [
            #     x for l, a in zip(linvel_sensor_names, angvel_sensor_names)
            #     for x in (
            #             self.mjc_data.sensor(l).data.tolist()
            #             + self.mjc_data.sensor(a).data.tolist()
            #     )
            # ]
...
if __name__ == '__main__':
    ...
    exp_dir = Path(base_path, "ex_bar_drop/raw_data/")
    xml_path = Path("xml_models/single_rod_motor_endcaps.xml")
    exp_dir.parent.mkdir(exist_ok=True)
    exp_dir.mkdir(exist_ok=True)

    sim = AbstractMuJoCoSimulator(xml_path, visualize=True)

    sim.mjc_data.qvel = [2.0, -2.0, 0.0, 0.0, 0.0, 0.0]
